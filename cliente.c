...
// >>>>>>>>> ALTERAÇÃO: Constante TAM_MEMORIA foi adicionada para corrigir o erro <<<<<<<<<<<<<
#define TAM_BUFFER 6
...
void intercalacao_otima_cliente(char *nome_base_particao, int num_part, char *nome_final) {
    ...
    for (int i = 0; i < num_part; i++) {
        char nome_part[100];
        sprintf(nome_part, "%s%d.dat", nome_base_particao, i);
        // A verificação 'if(particoes[i])' não é segura aqui pois o ponteiro pode ser inválido
        // A lógica de fechar os arquivos já abertos é mais segura
    ...
    // Fecha os ponteiros que ainda estão abertos
    for(int i = 0; i < num_part; ++i){
...
void cria_clientes_desordenado(FILE *out, int tam) {
    ...
    for (int i = 0; i < tam; i++) {
        char nome_temp[50];
        // Gera IDs num intervalo maior para reduzir colisões
        int id = rand() % (tam * 5);
