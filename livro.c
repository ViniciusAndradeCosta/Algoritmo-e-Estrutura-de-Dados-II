...
void atualiza_livro_txt(FILE *dat_in) {
    ...
    while ((l = le_livro(dat_in)) != NULL) {
        ...
        fprintf(txt_out, "Preço: R$ %.2f\n", l->preco);
        fprintf(txt_out, "Páginas: %d\n", l->paginas);
        fprintf(txt_out, "Edição: %d\n", l->edicao);
...
void busca_sequencial_livro(FILE *in) {
    ...
    if (log_file != NULL) {
        fprintf(log_file,
                "Tipo de Busca: Sequencial; Código Procurado: %d; Encontrado: %s; "
                "Comparações: %d; Tempo: %f segundos\n",
...
TLivro *busca_binaria_livro(FILE *in) {
    ...
    if (log_file != NULL) {
        fprintf(log_file,
                "Tipo de Busca: Binária; Código Procurado: %d; Encontrado: %s; "
                "Comparações: %d; Tempo: %f segundos\n",
                n, encontrado ? "Sim" : "Nao", comp, tempoTotal);
        fclose(log_file);
    }
...
//FUNÇÕES DE BUSCA INTERNA DO SISTEMA
...
void cria_livros_desordenado(FILE *out) {
    ...
    for (int i = 0; i < tam; i++) {
        char titulo_temp[50];
        sprintf(titulo_temp, "Livro de Teste %d", vet[i]);
        TLivro *l = livro_cad(20.0 + (rand() % 80), vet[i], 150 + (rand() % 200), 1 + (rand() % 3), 5 + (rand() % 10),
                              titulo_temp, "Autor Fictício", "Editora Exemplo");
        if (l != NULL) {
            salva_livro(l, out);
            free(l);
        }
